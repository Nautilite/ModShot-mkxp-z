...
int main(int argc, char *argv[])
{
...
#ifdef MKXPZ_STEAM
	if (!STEAMSHIM_init()) {
		showInitError("Failed to initialize Steamworks");
		SDL_Quit();
		return 0;
	}
#endif
	...
	if (WSAStartup(0x101, &wsadata) || wsadata.wVersion != 0x101) {
		...
		showInitError(std::string(buf));
		// Not an error worth ending the program over
	}
#endif
	...
	if (!win) {
		showInitError(std::string("Error creating SDL window: ") + SDL_GetError());
#ifdef MKXPZ_STEAM
		STEAMSHIM_deinit();
#endif
		return 0;
...
#ifdef MKXPZ_BUILD_XCODE
#define DEBUG_FSELECT_MSG "Select the folder from which to load game files."
#define DEBUG_FSELECT_PROMPT "Load Game"
	...
	RGSSThreadData rtData(
		&eventThread, argv[0], win, alcDev,
		mode.refresh_rate, mkxp_sys::getScalingFactor(),
		conf, glCtx
	);
...
static SDL_GLContext initGL(SDL_Window *win, Config &conf, RGSSThreadData *threadData)
{
	...
	if (!glCtx) {
		GLINIT_SHOWERROR(std::string("Error creating GL context: ") + SDL_GetError());
		return 0;
	}
	...
	// Print OpenGL hardware versions
	printGLInfo();
