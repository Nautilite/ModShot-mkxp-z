			...
			default:
				...
				switch(event.type - usrIdStart)
				{
					...
					case REQUEST_WINCENTER:
						...
						if (!rc)
							SDL_SetWindowPosition(win, (dm.w / 2) - (winW / 2), (dm.h / 2) - (winH / 2));
						rtData.rqWindowAdjust.clear();
						break;
					...
					case REQUEST_MESSAGEBOX:
					{
						...
						std::string message = copyWithNewlines(
							(const char*)event.user.data1, 70
						);
...
void SyncPoint::haltThreads()
{
	...
	/* Now that the RGSS thread is asleep, we can safely put
	   the other threads to sleep as well without causing deadlocks
	*/
	secondSync.lock();
}
