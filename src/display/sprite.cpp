...
struct SpritePrivate
{
	...
	void onSrcRectChange()
	{
		...
		if (mirrored && vmirrored)
			quad.setTexRect(rect.hvFlipped());
		else if (mirrored)
			quad.setTexRect(rect.hFlipped());
		else if (vmirrored)
			quad.setTexRect(rect.vFlipped());
		else
	...
	void emitWaveChunk(SVertex *&vert, float phase, int width, float zoomY, int chunkY, int chunkLength)
	{
		...
		if (mirrored && vmirrored)
			Quad::setTexPosRect(vert, tex.hvFlipped(), pos);
		else if (mirrored)
			Quad::setTexPosRect(vert, tex.hFlipped(), pos);
		else if (vmirrored)
			Quad::setTexPosRect(vert, tex.vFlipped(), pos);
		else
