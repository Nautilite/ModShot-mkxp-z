...
void CompiledShader::compileShader(const char *contents, GLuint shader, GLuint program, bool vert)
{
	...
	// This conditional assumes that vertex shaders are handled
	// after fragment shaders. Alternatively, one may call LinkProgram()
	// and do the error checking in every place where this compileShader()
	// method is called.
	if (vert) {
...
CustomShader::CustomShader(CompiledShader *shader, VALUE args, VALUE texUnits)
	: shader(shader),
